...
wget -N --no-check-certificate -O x-ui-custom.zip "https://github.com/bestalosh/3x-ui/releases/download/v1.0/x-ui-custom-release.zip"
...
if [[ "$core_choice" == "1" ]]; then
    ...
    mv bin/xray-standard bin/xray-linux-amd64
    rm -f bin/xray-custom
else
    echo "Preparing Custom Protected Core..."
    mv bin/xray-custom bin/xray-linux-amd64
