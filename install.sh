#!/bin/bash
# Based on MHSanaei/3x-ui v3.7.0 install.sh; upstream licensing applies.
# Source: https://github.com/MHSanaei/3x-ui/blob/v3.7.0/install.sh
# Customization: fixed Sanaei v3.7.0 + Normal/Protected archive selection.
# Protected uses the release marked Latest in bestalosh/3x-ui on each run.
# Assets: xray-linux-ARCH.tar.gz, containing xray-linux-ARCH.
PROTECTED_RELEASE_BASE="https://github.com/bestalosh/3x-ui/releases/latest/download"

if [[ $# -gt 1 || ( $# -eq 1 && "$1" != "v3.7.0" && "$1" != "3.7.0" ) ]]; then
    echo "This installer supports only 3x-ui v3.7.0. Run: bash install.sh" >&2
    exit 1
fi
...
# BEGIN CUSTOM VARIANT FUNCTIONS
choose_install_variant() {
    local choice
    printf '\n1. Normal 3x-ui\n2. Protected 3x-ui\n'
    while true; do
        if [[ "${NONINTERACTIVE:-0}" == 1 ]]; then
            choice="${XUI_INSTALL_MODE:-1}"
        elif ! read -rp "Select installation type [1]: " choice; then
            echo "Cannot read selection. Download install.sh and run: bash install.sh" >&2
            return 1
        fi
        case "${choice:-1}" in
            1) INSTALL_VARIANT=normal; return 0 ;;
            2) INSTALL_VARIANT=protected; return 0 ;;
            *)
...
download_protected_xray() {
    local cpu="$1" destination="$2" status url
    # GitHub resolves this endpoint to the release marked Latest on every run.
    url="${PROTECTED_RELEASE_BASE%/}/xray-linux-${cpu}.tar.gz"
    if status=$(curl -fL --retry 3 --connect-timeout 15 --max-time 600 \
        -w '%{http_code}' -o "$destination" "$url"); then
        return 0
    fi
    echo "Protected Xray download failed for ${cpu} (HTTP ${status:-unknown})." >&2
    return 1
}

# Modify the DOWNLOADED archive before the original installer continues.
# A subshell confines cleanup traps to this operation.
protect_downloaded_archive() (
    local archive="$1" cpu="$2" protected_work target replacement custom_archive
    if [[ "$PROTECTED_RELEASE_BASE" == *YOUR_USERNAME* || "$PROTECTED_RELEASE_BASE" == *YOUR_REPO* ]]; then
        echo "Set PROTECTED_RELEASE_BASE at the top of install.sh first." >&2
        return 1
    fi
    # Use the archive's filesystem so the final rename is atomic.
    protected_work=$(mktemp -d "$(dirname "$archive")/.xui-protected.XXXXXX") || return 1
    trap 'rm -rf -- "$protected_work"' EXIT
    trap 'exit 130' INT
    trap 'exit 143' TERM
    mkdir "$protected_work/tree" || return 1
    if ! tar -xzf "$archive" -C "$protected_work/tree"; then
        echo "Cannot extract the downloaded 3x-ui archive." >&2
        return 1
    fi
    target="$protected_work/tree/x-ui/bin/xray-linux-${cpu}"
    if [[ ! -f "$target" || -L "$target" ]]; then
        echo "Expected binary is missing: x-ui/bin/xray-linux-${cpu}" >&2
        return 1
    fi
    replacement="$protected_work/custom-xray"
    custom_archive="$protected_work/custom-xray.tar.gz"
    echo "Downloading your latest Protected Xray: ${cpu}"
    if ! download_protected_xray "$cpu" "$custom_archive"; then
        echo "Protected Xray download failed. Installation stopped." >&2
        return 1
    fi
    # The Release archive contains one raw executable named xray-linux-ARCH.
    # Extract only that member to stdout, never arbitrary paths from the archive.
    if ! tar -xOzf "$custom_archive" "xray-linux-${cpu}" > "$replacement"; then
        echo "Cannot extract xray-linux-${cpu} from the Protected archive." >&2
        return 1
    fi
    if [[ ! -s "$replacement" ]] || ! chmod 755 "$replacement"; then
        echo "Protected Xray is empty or cannot be made executable." >&2
        return 1
    fi
    if ! "$replacement" -version; then
        echo "Protected Xray cannot run on this server (-version failed)." >&2
        return 1
    fi
    # Copy onto the existing regular file to retain its original owner/group.
    cp -- "$replacement" "$target" && chmod 755 "$target" || return 1
    # Keep all extracted contents, including any additional top-level files.
    if ! tar -czf "$protected_work/repacked.tar.gz" -C "$protected_work/tree" .; then
        echo "Failed to rebuild the 3x-ui archive." >&2
        return 1
    fi
    if ! mv -f -- "$protected_work/repacked.tar.gz" "$archive"; then
        echo "Failed to replace the downloaded archive." >&2
        return 1
    fi
    echo "Downloaded archive updated with your Protected Xray. Continuing installation..."
)
# END CUSTOM VARIANT FUNCTIONS

install_x-ui() {
    cd "${xui_folder%/x-ui}/" || exit 1

    # Always download the original Sanaei v3.7.0 release for this server.
    tag_version="v3.7.0"
    local cpu archive url
    cpu=$(arch) || exit 1
    archive="${xui_folder}-linux-${cpu}.tar.gz"
    url="https://github.com/MHSanaei/3x-ui/releases/download/${tag_version}/x-ui-linux-${cpu}.tar.gz"
    echo "Downloading original Sanaei 3x-ui ${tag_version} (${cpu})..."
    if ! curl -fLR --retry 5 --retry-delay 3 --connect-timeout 15 \
        --speed-limit 1 --speed-time 300 -o "$archive" "$url"; then
        echo "Failed to download Sanaei 3x-ui ${tag_version}." >&2
        exit 1
    fi
    if [[ ! -s "$archive" ]]; then
        echo "Downloaded 3x-ui archive is empty." >&2
        exit 1
    fi

    # Select after download and modify the package BEFORE installation continues.
    choose_install_variant || exit 1
    if [[ "$INSTALL_VARIANT" == protected ]]; then
        protect_downloaded_archive "$archive" "$cpu" || exit 1
    fi

    local xui_script_temp="/usr/bin/x-ui-temp.$$"
    rm -f "${xui_script_temp}"
    curl -fLRo "${xui_script_temp}" https://raw.githubusercontent.com/MHSanaei/3x-ui/v3.7.0/x-ui.sh
    if [[ $? -ne 0 ]]; then
        rm -f "${xui_script_temp}"
    ...
    if [[ $release == "alpine" ]]; then
        ...
        rm -f "${xui_rc_temp}"
        curl -fLRo "${xui_rc_temp}" https://raw.githubusercontent.com/MHSanaei/3x-ui/v3.7.0/x-ui.rc
        if [[ $? -ne 0 ]]; then
            rm -f "${xui_rc_temp}"
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    service_unit_url="https://raw.githubusercontent.com/MHSanaei/3x-ui/v3.7.0/x-ui.service.debian"
                    ;;
                arch | manjaro | parch)
                    service_unit_url="https://raw.githubusercontent.com/MHSanaei/3x-ui/v3.7.0/x-ui.service.arch"
                    ;;
                *)
                    service_unit_url="https://raw.githubusercontent.com/MHSanaei/3x-ui/v3.7.0/x-ui.service.rhel"
                    ;;
            esac
